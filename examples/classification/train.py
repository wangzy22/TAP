...
def main(gpu, cfg, profile=False):
    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            ...
            if cfg.mode == 'test':
                ...
                # epoch, best_val = load_checkpoint( # for t-sne vis
                #     model, pretrained_path=cfg.pretrained_path, prefix='encoder.')
                ...
                # finetune the whole model
                logging.info(f'Finetuning Encoder from {cfg.pretrained_path}')
                pretrained_keys = load_checkpoint(model.encoder, cfg.pretrained_path, return_pretrained_keys=True)
                logging.info(f'Pretrained keys {pretrained_keys}')
    else:
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if cfg.mode == 'finetune_encoder':
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch(model, train_loader,
                                optimizer, scheduler, epoch, cfg, pretrained_keys)
        else:
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch(model, train_loader,
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg, pretrained_keys=None):
    ...
    for idx, data in pbar:
        ...
        if num_iter == cfg.step_per_update:
            if cfg.get('grad_norm_clip_encoder') is not None:
                assert pretrained_keys is not None
                param_list = []
                for name, param in model.encoder.named_parameters():
                    if name in pretrained_keys:
                        param_list.append(param)
                torch.nn.utils.clip_grad_norm_(
                    param_list, cfg.grad_norm_clip_encoder, norm_type=2)
            if cfg.get('grad_norm_clip') is not None and cfg.grad_norm_clip > 0.:
                if cfg.get('grad_norm_clip_encoder') is not None:
                    param_list = []
                    for name, param in model.named_parameters():
                        if name[len('encoder.'):] not in pretrained_keys:
                            param_list.append(param)
                    torch.nn.utils.clip_grad_norm_(
                        param_list, cfg.grad_norm_clip, norm_type=2)
                else:
                    torch.nn.utils.clip_grad_norm_(
...
@torch.no_grad()
def validate_tsne(model, val_loader, cfg):
    model.eval()  # set model to eval mode
    cm = ConfusionMatrix(num_classes=cfg.num_classes)
    npoints = cfg.num_points
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__())
    ...
    for idx, data in pbar:
        ...
        data['pos'] = points[:, :, :3].contiguous()
        data['x'] = points[:, :, :cfg.model.in_channels].transpose(1, 2).contiguous()
        logits = model.forward_feat(data)  # for t-sne vis
        test_logits.append(logits.detach().cpu().numpy())
        test_label.append(target.cpu().numpy())

    test_label = np.concatenate(test_label)
    test_logits = np.concatenate(test_logits)

    from sklearn.manifold import TSNE
    X_embedded = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(test_logits)
    np.save('pointMLP_TAP_sn15_unsup.npy', X_embedded)
    np.save('pointMLP_TAP_sn15_label_unsup.npy', test_label)
    # np.save('pointMLP_TAP_mn40_unsup.npy', X_embedded)
    # np.save('pointMLP_TAP_mn40_label_unsup.npy', test_label)\
    return 