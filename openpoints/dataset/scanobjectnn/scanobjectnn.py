...
class ScanObjectNN(Dataset):
    ...
    def __init__(self, data_dir, split, mode='hardest',
                 num_points=2048,
                 uniform_sample=True,
                 transform=None,
                 **kwargs):
        ...
        if mode == 'objectbg' or mode == 'objectonly':
            h5_name = os.path.join(data_dir, f'{slit_name}_objectdataset.h5')
        elif mode == 'hardest':
            h5_name = os.path.join(
                data_dir, f'{slit_name}_objectdataset_augmentedrot_scale75.h5')
        else:
            raise NotImplementedError(f'ScanObjectNN dataset mode {mode} not implemeted!')
        ...
        if slit_name == 'test' and uniform_sample:
            if mode == 'objectbg' or mode == 'objectonly':
                precomputed_path = os.path.join(
                    data_dir, f'{slit_name}_objectdataset_{num_points}_fps.h5')
            elif mode == 'hardest':
                precomputed_path = os.path.join(
                    data_dir, f'{slit_name}_objectdataset_augmentedrot_scale75_{num_points}_fps.pkl')
            if not os.path.exists(precomputed_path):
                points = torch.from_numpy(self.points).to(torch.float32).cuda()
                self.points = fps(points, num_points).cpu().numpy()
        ...
        logging.info(f'Successfully load ScanObjectNN {mode} {split} '
